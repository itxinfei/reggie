...
| 📦 **全业务覆盖** | 堂食 + 进销存 + 会员 + 支付 + 配送 + 打印 + 报表 + 营销 + 通知 + 多门店 + 导出 |
...
| 📊 经营报表 | 销售统计/营业分析 | 日/周/月报表、菜品销量排行、时段分析 |
| 🎯 营销推荐 | 推荐引擎/营销活动 | 协同过滤推荐、偏好分析、营销活动、批量推送 |
...
│   │   ├── recommend/   # 🎯 营销推荐（推荐引擎、营销活动、消息推送）
│   │   ├── report/      # 📊 报表（销售统计、营业分析）
│   │   ├── store/       # 🏪 多门店（门店信息、数据隔离、仪表盘）
│   │   ├── notification/# 🔔 消息通知（模板管理、短信/APP推送、多渠道路由）
│   │   └── export/      # 📥 数据导出（Excel、PDF 多格式导出）
