...
<img src="https://img.shields.io/badge/Modules-43-1677ff?logo=spring" alt="43 Modules">
<img src="https://img.shields.io/badge/Data_Tables-50+-ff6b6b?logo=postgresql" alt="50+ Tables">
<img src="https://img.shields.io/badge/Java_Files-629-4379a7?logo=java" alt="629 Java files">
<img src="https://img.shields.io/badge/Total_Commits-246-success?logo=git" alt="246 commits">
...
**瑞吉外卖**是一套完整的餐饮管理系统，基于 Spring Boot + Vue 的单体应用架构，覆盖堂食、外卖、进销存、会员、支付、打印、报表等餐饮全业务场景。系统定位为 **品牌直营外卖系统 + 品牌连锁管理系统**，私域（自有小程序/APP）与多平台渠道（美团/饿了么/抖音）同步运营。系统核心创新在于 **AI 智能引擎**，通过接入大语言模型实现智能点餐推荐、菜品描述生成、经营分析等能力。


### 🏗️ 模块化架构

项目采用 **模块化设计**，所有业务功能按领域划分为 43 个独立模块，每个模块包含完整的 Controller → Service → Mapper → Model 分层：
...
| 🎁 **marketing** | 营销管理 | 秒杀、满减、买赠、拼团 |
| 📢 **notification** | 通知管理 | 消息推送、模板 |
| 🚚 **delivery** | 配送管理 | 美团/饿了么/抖音平台对接 |
| 🪑 **dining** | 堂食管理 | 桌台、排队、预约、叫号 |
| 🛎️ **customer** | 客服管理 | 客户服务、工单、投诉 |
| 📤 **export** | 数据导出 | Excel/PDF 导出 |
| ⏰ **schedule** | 定时任务 | 订单超时、数据统计 |
| 📋 **sys** | 系统管理 | 角色、权限、配置、日志 |
| 🌍 **region** | 区域管理 | 省市区数据 |
| 🔧 **common** | 公共接口 | 文件上传、餐厅信息 |
| 🤝 **franchise** | 加盟管理 | 加盟商、合同、结算 |
| 👔 **attendance** | 考勤管理 | 打卡、排班 |
| ⏱️ **urgency** | 催单管理 | 订单催办、配送催单 |
| 🔄 **retention** | 用户留存 | 流失预警、召回策略 |
...
| 📱 **双端覆盖** | 管理后台（Element UI）+ 移动端（Vant UI / H5）+ 多平台渠道（美团/饿了么/抖音） |
| 🔐 **多租户 SaaS** | 行级数据隔离，一套系统服务多家门店 |
| ⚡ **前后端一体** | 前端页面内嵌于 Spring Boot，单 Jar 部署，无需分离部署 |
| 📦 **全业务覆盖** | 堂食 + 外卖配送 + 进销存 + 会员营销 + 支付 + 打印 + 报表 + 加盟管理 + 多平台渠道 |
...
| 依赖 | 版本 | 说明 |
|------|------|------|
| ☕ JDK | 8 | 必须 JDK 8，已配置 maven-enforcer + animal-sniffer 双保险拦截高版本 |
...
| 🪑 **堂食管理** | 桌台/区域管理、预订跟踪、取号排队、桌台状态实时看板 |
| 📦 **进销存** | 原料管理、供应商管理、采购入库、库存盘点、库存记录追踪 |
| 💹 **财务管理** | 收支明细、利润分析、成本核算 |
| 👔 **考勤管理** | 员工打卡、排班管理 |
...
│   ├── enums/            # 状态枚举（13 个）
│   ├── filter/           # 登录拦截过滤器
│   ├── module/           # 🧩 业务模块（35 个，各含 controller/service/mapper/model）
│   │   ├── ai/           #   🤖 AI智能引擎（点餐推荐/描述生成/经营分析/对话管理/用户画像）
│   │   ├── franchise/    #   🤝 加盟管理（加盟商/合同/分账结算）
│   │   ├── delivery/     #   🚚 外卖配送（美团/饿了么/抖音平台对接）
...
│   │   ├── marketing/    #   🎁 营销（秒杀/满减/买赠/拼团）
│   │   ├── finance/      #   💹 财务（收支/利润/成本）
│   │   ├── cashier/      #   💳 收银（收银记录/日结）
│   │   ├── notification/ #   🔔 消息通知（模板/短信/推送/多渠道路由）
│   │   ├── export/       #   📥 数据导出（Excel/PDF多格式）
│   │   ├── customer/     #   🛎️ 客服（会话/工单/投诉）
│   │   ├── attendance/   #   👔 考勤（打卡/排班）
│   │   ├── retention/    #   🔄 用户留存（流失预警/召回）
│   │   ├── urgency/      #   ⏱️ 催单（订单催办/配送催单）
│   │   ├── dashboard/    #   📊 仪表盘（经营概览/实时数据）
│   │   ├── sys/          #   📋 系统管理（角色/权限/配置/日志）
│   │   └── ...           #   另有 user/dish/order/category/setmeal/address/region/schedule 等
│   ├── service/          # 业务接口 + 实现类
│   └── utils/            # 工具类（二维码、验证码、SMS、文件操作）
├── src/main/resources/
│   ├── backend/          # 🖥️ 管理后台（Element UI，74 个页面）
│   ├── front/            # 📱 移动端（Vant UI，15 个页面）
│   ├── scripts/          # 📜 数据库初始化脚本
...
| 📦 **扩展模块** | 堂食、进销存、会员、支付、配送、打印、报表、加盟、营销、财务、收银、客服（35 个模块） |
| 🤝 **加盟连锁** | 加盟商管理、合同签署、分账结算 |
| 🎁 **营销管理** | 秒杀、满减、买赠、拼团活动 |
| 🚚 **多平台渠道** | 美团/饿了么/抖音平台对接，工厂模式适配 |
...
**629** Java 源文件 · **74** 管理后台页面 · **43** 业务模块 · **246** 次提交
