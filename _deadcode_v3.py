...
# Cat1: Interface method + Impl method, but no external caller (excl test/ctrl/self)
cat1 = []
    ...
    for im in iface_methods:
        ...
        for p, content in file_contents.items():
            if p == impl_file or p == iface_file:
                continue
            if p in test_files or p in controller_files:
                continue
...
print(f"CAT1 (interface method, impl exists, no external caller excl ctrl/test): {len(cat1)}")
for c in cat1:
