...
public class ElemeAdapter extends AbstractDeliveryPlatform {
    ...
    public boolean verifyCallback(Map<String, String> params) {
        ...
        if (config.isMockMode()) {
            log.warn("[饿了么] 回调签名校验已跳过（mock-mode=true，仅限开发/演示），生产环境必须关闭 mock-mode 并配置 notify-token");
            return true;
        }
        DeliveryPlatformConfigProperties.PlatformConfig pc = getPlatformConfig();
        if (pc == null || isBlank(pc.getNotifyToken())) {
            log.error("[饿了么] 回调签名校验失败：平台未配置 notify-token（fail-closed）");
            return false;
        }
        String sign = params.get("sign");
        if (sign == null || sign.trim().isEmpty()) {
            log.warn("[饿了么] 回调缺少 sign 参数");
            return false;
        }
