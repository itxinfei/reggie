...
def extract_interface_methods_v2(content):
    methods = []
    lines = content.split('\n')
...
def extract_impl_methods_v2(content):
    methods = []
    lines = content.split('\n')
    class_name = None
...
controller_files = set(p for p in all_java if "/controller/" in p.lower())
...
for impl_file, iface_file in paired.items():
    ...
    iface_methods = extract_interface_methods_v2(iface_content)
    for im in iface_methods:
        name = im['name']
        has_external = False
        ref_files = []
        for p, content in file_contents.items():
            if p == impl_file or p in test_files or p in controller_files:
                continue
        ...
        if not has_external:
            cat1.append({'impl_file':os.path.basename(impl_file),'iface_file':os.path.basename(iface_file),'method':name,'return_type':im['return_type'],'params':im['params'],'line':im['line'],'signature':im['signature'],'refs':ref_files})
...
for impl_file, iface_file in paired.items():
    ...
    iface_content = file_contents.get(iface_file, '')
    iface_methods = extract_interface_methods_v2(iface_content)
    impl_methods = extract_impl_methods_v2(impl_content)
